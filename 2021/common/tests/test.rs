...
mod tests {
    ...
    fn input_readable_by_file_to_vec() {
        let my_vec = common::file_to_vec("input.txt".to_string()).unwrap();
        assert!(my_vec.len() > 0);
    }

    #[test]
    fn input_readable_by_file_to_vec_of_char_vec() {
        let my_vec = common::file_to_vec_of_char_vec("input.txt".to_string());
        assert!(my_vec.len() > 0);
    }

    #[test]
    fn input_readable_by_file_to_vec_of_words_vec() {
        let my_vec = common::file_to_vec_of_words_vec("input2.txt".to_string());
        assert!(my_vec.len() > 0);
    }
